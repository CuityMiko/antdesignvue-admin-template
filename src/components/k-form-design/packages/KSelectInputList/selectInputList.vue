...
<template>
  <a-form-model class="select-input-list-box" ref="dynamicValidateForm" layout="inline" :model="dynamicValidateForm">
    <div v-for="(column, i) in record.columns" :key="i">
      <a-form-model-item>
        <a-checkbox v-if="record.options.multiple" @change="onCheckboxChange($event, i)" :checked="dynamicValidateForm.domains[i].checked">
          {{ column.label }}
        </a-checkbox>
        <a-radio v-else @change="onRadioChange($event, i)" :checked="dynamicValidateForm.domains[i].checked">{{ column.label }}</a-radio>
      </a-form-model-item>
